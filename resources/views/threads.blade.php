                                        ...
                                        <div class="w-1/5 px-4 flex items-center">
                                            <div class="w-10">
                                                <img class="inline-block h-10 w-10 rounded-full"
                                                     src="{{ 'https://s.gravatar.com/avatar/'.md5(strtolower(trim(isset($thread->to()->email) ?  $thread->to()->email : 'taylor@laravel.com' ))).'?s=80' }}"
                                                     alt="">
                                            </div>
                                        </div>
                                        <div class="w-3/4 px-4 flex items-center">
                                            <div class="container flex-col">
                                                <div class="flex justify-between text-gray-500">
                                                    <div class="text-sm mb-2">{{ $thread->to()->name }}</div>
                                                    <div
                                                        class="text-sm text-right">{{ Carbon\Carbon::parse($thread->updated_at)->format('H:m')}}</div>
                                                </div>
                                                @if ($activeThread->messages()->count() == 0 && $thread->id == $activeThread->id)
                                                    <div class="text-base">Create new message</div>
                                                @else
